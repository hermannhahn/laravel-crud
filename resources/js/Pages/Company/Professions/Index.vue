...
const updateFilters = () => {
    router.get(route('professions.index'), {
        search: search.value,
...
const deleteProfession = (id: number) => {
    if (confirm('Are you sure you want to delete this profession? This might affect tasks and services in this profession.')) {
        router.delete(route('professions.destroy', id));
    }
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Professions Catalog</h2>
                <Link :href="route('professions.create')">
                    <PrimaryButton>Add Profession</PrimaryButton>
                </Link>
            </div>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        <div v-if="professions.data.length === 0" class="text-center py-8">
                            <p class="text-gray-500 dark:text-gray-400">No professions found.</p>
                        </div>
                        ...
                        <div v-else class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-900">
                                    <tr>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Profession Name</th>
                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    <tr v-for="prof in professions.data" :key="prof.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150">
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ prof.name }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end space-x-2">
                                                <Link 
                                                    :href="route('professions.edit', prof.id)" 
                                                ...
                                                <button 
                                                    @click="deleteProfession(prof.id)" 
                        ...
                        <!-- Pagination -->
                        <div v-if="professions.links && professions.links.length > 3" class="mt-6 flex justify-center">
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                                <Link 
                                    v-for="(link, index) in professions.links" 
                                    :key="index"
                                    :href="link.url || '#'"
                                    v-html="link.label"
